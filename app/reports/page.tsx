...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setSalaries(sal); setEmployees(emp);
      setLoading(false);
    });
  }, [authChecked]);
  ...
  const monthStats = useMemo(() => {
    ...
    // Revenue = sum of all payments this month
    const mPayments = payments.filter((p) => p.date?.startsWith(prefix));
    const totalRevenue = mPayments.reduce((sum, p) => sum + p.amount, 0);

    // Expenses (supplies + rent only)
    ...
    // Salaries from salaries collection only
    ...
    const totalSalaries = mSalaries.reduce((sum, s) => sum + s.amount, 0);

    // Net profit = revenue - expenses - salaries
    const netProfit = totalRevenue - totalExpenses - totalSalaries;

    // Expenses by type
    const expByType: Record<string, number> = { supplies: 0, rent: 0 };
    for (const e of mExpenses) {
      expByType[e.type] = (expByType[e.type] ?? 0) + e.amount;
    }

    // Salary rows with employee info
    const salaryRows = mSalaries
      .map((s) => ({ ...s, employee: employeesMap[s.employeeId] }))
      .filter((s) => s.employee)
      .sort((a, b) => b.amount - a.amount);

    return { totalRevenue, totalExpenses, totalSalaries, netProfit, expByType, salaryRows };
  }, [selMonth, selYear, payments, expenses, salaries, employeesMap]);
  ...
  const chartData = useMemo(() => {
    ...
    return months.map(({ month, year }) => {
      ...
      const revenues = payments.filter((p) => p.date?.startsWith(prefix))
        .reduce((sum, p) => sum + p.amount, 0);
      const totalExp = expenses.filter((e) => e.month === month && e.year === year)
        .reduce((sum, e) => sum + e.amount, 0);
      const totalSal = salaries.filter(
  ...
  const { totalRevenue, totalExpenses, totalSalaries, netProfit, expByType, salaryRows } = monthStats;
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              <h3 className="text-sm font-semibold text-gray-800 mb-4">مقارنة آخر 6 أشهر</h3>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}
                  ...
                  <Tooltip
                    formatter={(value, name) => [`${Math.round(Number(value)).toLocaleString()} جنيه`, name]}
                  ...
                  <Bar dataKey="costs" name="المصاريف والمرتبات" fill="#3b82f6" radius={[4, 4, 0, 0]}>
                    {chartData.map((entry, i) => (
              ...
              <Section title="المصاريف حسب النوع">
                {totalExpenses === 0 ? (
                  ...
                  <div className="space-y-3">
                    {(["supplies", "rent"] as const).map((type) => {
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-800">طلاب لم يدفعوا كامل المبلغ</h3>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {studentsWithBalance.map((s) => (
                        <tr key={s.id} className="hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{s.totalAmount.toLocaleString()} ج</td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span className="font-medium text-green-700">{s.paidAmount.toLocaleString()} ج</span>
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span className="font-semibold text-orange-600">{s.remainingAmount.toLocaleString()} ج</span>
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap">
                            <a href={`https://wa.me/2${(s as unknown as { phone: string }).phone}`}
